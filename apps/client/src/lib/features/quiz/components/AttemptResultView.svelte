		...
		<div class="flex flex-wrap items-start justify-between gap-3">
			<div>
				...
				<p class="mt-2 text-sm text-zinc-700">
					Enviado: {formatDate(result.submittedAt)} - Evaluado: {formatDate(result.evaluatedAt)}
				</p>
			</div>
			<button class="btn-secondary" type="button" onclick={() => quizUiStore.clearAllStores()}>
				Volver al panel
			</button>
		</div>
			...
			{#each result.questions as question, index}
				<article class="panel-muted p-4 sm:p-5">
					...
					<p class="mt-4 mb-0 text-sm text-zinc-700">
						Tu respuesta: {question.answerIndex === null ? '-' : question.answerIndex + 1} - Respuesta correcta:
						{question.correctAnswerIndex + 1} - Puntos: {formatPoints(question.awardedPoints)}
					</p>
				</article>
			{/each}
		</div>
